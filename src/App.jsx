...
export default function App() {
  ...
  const [terminalVista, setTerminalVista] = useState("menu"); // menu | thm
  ...
  const [progresoT,     setProgresoT]     = useLocalStorage(`hf_progresoT_${userKey}`,      { xp: 0, completados: [] });

  const totalXp = 1240 + labsXp + (progresoT?.xp || 0);
  ...
  const completarTerminal = (id, xp) => {
    setProgresoT(p => ({
      xp: (p?.xp || 0) + xp,
      completados: [...(p?.completados || []), id]
    }));
  };
  ...
  const resetProgress = () => {
    if (window.confirm("¿Resetear todo el progreso?")) {
      ...
      setProgresoMods({}); setProgresoT({ xp: 0, completados: [] });
    }
  };
  ...
  const handleNav = (id) => {
    ...
    if (id === "terminal") setTerminalVista("menu");
    ...
    <div style={{ display:"flex", height:"100vh", overflow:"hidden", background:C.bg, color:"#c9d1d9" }}>
        ...
        {[
         ...
         { id:"rt", icon:"🛠️", label:"Herramientas" },
          ...
          <div key={n.id} className="nav-item"
            ...
            {n.id==="terminal" && (progresoT?.xp || 0) > 0 && <span style={{ marginLeft:"auto", color:"#00d4ff", fontSize:10 }}>{progresoT.xp}xp</span>}
          </div>
        ))}
      ...
      <main className="main-content" style={{ flex:1, overflowY:"auto", padding:"24px" }}>
        ...
        {nav==="terminal" && terminalVista === "menu" && (
          <Terminal
            progresoT={progresoT}
            onCompletarT={completarTerminal}
            onIrTHM={() => setTerminalVista("thm")}
          />
        )}

        {nav==="terminal" && terminalVista === "thm" && (
          <THMSalas
            salas={THM_SALAS}
            onVolver={() => setTerminalVista("menu")}
            onCompletarSala={completarTerminal}
            xpTotal={progresoT?.xp || 0}
          />
        )}
